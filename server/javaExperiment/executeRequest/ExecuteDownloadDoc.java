...
public class ExecuteDownloadDoc extends Thread {
	boolean flag = true;
	DataInputStream dis;
	...
	public ExecuteDownloadDoc(Socket client) {
		super("excute download-");
		count++;
		this.client = client;
	}
	...
	public void run() {
		...
		try {
			...
			if (select.contains(Constants.FILE_FLAG)) {
				fis = new FileInputStream(Constants.DOCUMENT_FILE_CATLOG_PATH + select.replace(Constants.FILE_FLAG, ""));
